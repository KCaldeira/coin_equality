...
Test Cases:
1. When y_mean << y_damage_distribution_halfsat, Ω should approach ω_max (poor population)
2. When y_mean >> y_damage_distribution_halfsat, Ω should approach 0 (wealthy population)
3. When Gini → 0 and y_mean = y_damage_distribution_halfsat, Ω should approach ω_max/2 (equal incomes at half-saturation)
...
def test_edge_case_1_poor_population():
    """
    Test Case 1: y_mean << y_damage_distribution_halfsat → Ω ≈ ω_max
    ...
    """
    print("=" * 80)
    print("Edge Case 1: Poor Population (y_mean << y_damage_distribution_halfsat)")
    print("=" * 80)
    print("Expected: Ω → ω_max\n")

    # Set up parameters
    omega_max = 0.1  # 10% maximum damage
    y_damage_distribution_halfsat = 100000  # $100k half-saturation
    ...
    y_mean_values = [10000, 1000, 100, 10]  # Much smaller than y_damage_distribution_halfsat
    ...
    for y_mean in y_mean_values:
        ...
        print(f"  y_mean/y_damage_distribution_halfsat = {ratio:8.2e}    Ω = {Omega:.6f}    ω_max = {omega_max:.6f}    error = {error:.2e}")
    ...
    if final_error < tolerance:
        print(f"\n✓ PASS: For y_mean << y_damage_distribution_halfsat, Ω ≈ ω_max (error = {final_error:.2e})")
    else:
...
def test_edge_case_2_wealthy_population():
    """
    Test Case 2: y_mean >> y_damage_distribution_halfsat → Ω ≈ 0
    ...
    """
    print("=" * 80)
    print("Edge Case 2: Wealthy Population (y_mean >> y_damage_distribution_halfsat)")
    print("=" * 80)
    print("Expected: Ω → 0\n")
    ...
    y_mean_values = [10000, 100000, 1000000, 10000000]  # Much larger than y_damage_distribution_halfsat
    Gini = 0.4  # Moderate inequality
    ...
    for y_mean in y_mean_values:
        ...
        print(f"  y_mean/y_damage_distribution_halfsat = {ratio:8.2e}    Ω = {Omega:.6e}")

    # For the largest ratio, Omega should be very close to 0
    tolerance = 1e-3  # Omega should be < 0.001

    if Omega < tolerance:
        print(f"\n✓ PASS: For y_mean >> y_damage_distribution_halfsat, Ω → 0 (Ω = {Omega:.2e})")
    else:
...
def test_edge_case_3_equal_incomes_at_halfsat():
    """
    Test Case 3: Gini → 0 and y_mean = y_damage_distribution_halfsat → Ω ≈ ω_max/2

    When Gini approaches 0, we have perfect equality (all incomes equal).
    When all incomes equal y_damage_distribution_halfsat, everyone experiences damage:
        ω(y_damage_distribution_halfsat) = ω_max · y_damage_distribution_halfsat / (y_damage_distribution_halfsat + y_damage_distribution_halfsat)
                             = ω_max / 2
    Therefore aggregate damage should equal ω_max/2.
    """
    print("=" * 80)
    print("Edge Case 3: Equal Incomes at Half-Saturation (Gini → 0, y_mean = y_damage_distribution_halfsat)")
    print("=" * 80)
    print("Expected: Ω → ω_max/2\n")

    # Set up parameters
    omega_max = 0.1  # 10% maximum damage
    y_damage_distribution_halfsat = 50000  # $50k half-saturation
    y_mean = y_damage_distribution_halfsat  # Mean income equals half-saturation
    ...
    if final_error < tolerance:
        print(f"\n✓ PASS: For Gini → 0 and y_mean = y_damage_distribution_halfsat, Ω ≈ ω_max/2 (error = {final_error:.2e})")
    else:
        print(f"\n✗ FAIL: Error {final_error:.2e} exceeds tolerance {tolerance:.2e}")
