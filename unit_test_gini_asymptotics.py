...
2. ΔG small when y_mean >> y_damage_distribution_scale: Wealthy population, minimal damage
3. ΔG increases with ω_max: Larger damage amplifies inequality more
4. ΔG larger with larger G₀ at y_mean = y_damage_distribution_scale: More initial inequality to amplify
...
def test_gini_change_vanishes_with_perfect_equality():
    ...
    y_damage_distribution_scale. What matters is |ΔG| → 0.
    """
    print("=" * 80)
    ...
    y_damage_distribution_scale = 20000  # $20k half-saturation
    ...
    print(f"  y_mean = ${y_mean:,}, y_damage_distribution_scale = ${y_damage_distribution_scale:,}, ω_max = {omega_max:.2%}\n")
...
def test_gini_change_small_for_wealthy_population():
    """
    Test 2: |ΔG| small when y_mean >> y_damage_distribution_scale
    ...
    """
    print("=" * 80)
    print("Test 2: ΔG Small for Wealthy Population (y_mean >> y_damage_distribution_scale)")
    print("=" * 80)
    print("Expected: When y_mean >> y_damage_distribution_scale, ΔG should be small\n")
    ...
    print(f"  y_damage_distribution_scale = ${y_damage_distribution_scale:,}, G₀ = {Gini_initial}, ω_max = {omega_max:.2%}\n")
    ...
    for y_mean in y_mean_values:
        ...
        print(f"  y_mean/y_damage_distribution_scale = {ratio:8.1f}    ΔG = {delta_G:.6f}")
...
def test_gini_change_increases_with_damage():
    ...
    # Set up parameters with regressive damage
    # Use y_mean < y_damage_distribution_scale so damage amplifies inequality (regressive)
    y_damage_distribution_scale = 60000  # $60k half-saturation
    ...
    print(f"  y_mean = ${y_mean:,}, y_damage_distribution_scale = ${y_damage_distribution_scale:,}, G₀ = {Gini_initial}\n")
...
def test_gini_change_larger_with_higher_initial_gini():
    """
    Test 4: |ΔG| larger with larger G₀ at y_mean = y_damage_distribution_scale

    At y_mean = y_damage_distribution_scale, the population spans the full range of the
    ...
    print("=" * 80)
    print("Expected: At y_mean = y_damage_distribution_scale, ΔG should increase with G₀\n")

    # Set up parameters with mean income = half-saturation
    omega_max = 0.15  # 15% maximum damage
    y_damage_distribution_scale = 40000  # $40k half-saturation
    y_mean = y_damage_distribution_scale  # Mean equals half-saturation
    ...
    print(f"  y_mean = y_damage_distribution_scale = ${y_mean:,}, ω_max = {omega_max:.2%}\n")
    ...
    if is_increasing:
        print(f"\n✓ PASS: |ΔG| increases monotonically with G₀ at y_mean = y_damage_distribution_scale")
    else:
        print(f"\n✗ FAIL: |ΔG| does not increase monotonically with G₀")
        print(f"  |ΔG| values: {abs_delta_G_values}")
        raise AssertionError("Test 4 failed: |ΔG| should increase with G₀ when y_mean = y_damage_distribution_scale")
